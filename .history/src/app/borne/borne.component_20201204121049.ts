    ...
    if (this.format === 'image') {
      ...
      if (this.url !== undefined) {
        for (let i = 0; i < this.uploadForm.get('profile').value.length; i++) {
          this.myFiles.push(this.uploadForm.get('profile').value[i]);
        }

        const frmData = new FormData();
        for (let i = 0; i < this.file.length; i++) {
          frmData.append('fileUpload', this.uploadForm.get('profile').value[i]);
        }
        this.screensaverService.upload(frmData, id).subscribe(
          data2 => {
            console.log('ok');
          }
        );
