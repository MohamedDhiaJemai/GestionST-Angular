...
export class BorneComponent implements OnInit {
  ...
  onSelectFile(event, id) {
    this.eventToSend = event.target.files[0];
    console.log('event To Send', this.eventToSend)
  ...
  upload(id) {
    console.log('event', this.eventToSend)
    if (this.format === 'image') {
      ...
      formData.append('profile', this.uploadForm.get('avaprofiletar').value);
      this.screensaverService.upload(formData, id).subscribe(
        (res) => console.log(res),
        (err) => console.log(err)
      );
