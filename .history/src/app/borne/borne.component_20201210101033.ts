...
export class BorneComponent implements OnInit {
  ...
  constructor(private borneService: BorneService, private messageService: MessageService,
    ...
    private modalService: BsModalService, private router: Router, private domSanitizer: DomSanitizer) {
    this.file = null;
  }
  ...
  showMaximizableDialog(id: number) {
    ...
    this.screensaverService.getImage(id).subscribe(data => {
      ...
      // this.ImgUrl = `data:image/png;base64,${{this.imageToShow}}`;
      this.ImgUrl = this.domSanitizer.bypassSecurityTrustUrl(myReader.result);

      this.testFile = this.sanitizer.bypassSecurityTrustUrl(this.imageToShow);
    }, error => {
      this.isImageLoading = false;
      console.log(error);
    });
