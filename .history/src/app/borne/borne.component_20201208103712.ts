    ...
    if (this.format === 'image') {
      ...
      if (this.url !== undefined) {
        ...
        for (let i = 0; i < this.uploadForm.get('profile').value.length; i++) {
          ...
          // console.log(`Uploading ${this.url[i].name} with size ${this.url[i].size} and type ${this.url[i].type}`);
          this.screensaverService.upload(this.uploadForm.get('profile').value[i], id);
        }
      }
      //   this.screensaverService.upload(formData, id).subscribe(
      //     data2 => {
