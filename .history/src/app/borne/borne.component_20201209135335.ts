...
export class BorneComponent implements OnInit {
  ...
  createImageFromBlob(image: Blob) {
    ...
    reader.addEventListener('load', () => {
       ...
       console.log(this.imageToShow.type)
    }, false);
    if (image) {
  ...
  showMaximizableDialog(id: number) {
    this.screensaverService.testFile(id).subscribe(
      data => {
        ;
        this.thumbnail = this.sanitizer.bypassSecurityTrustUrl(this.createImageFromBlob(data));

        console.log(this.createImageFromBlob)
      }
    );
