  ...
  upload(id) {
    ...
    if (this.format === 'image') {
      ...
      if (this.url !== undefined) {
        // const formData = new FormData();
        // formData.append('file', this.uploadForm.get('profile').value);
        // console.log('File Video ', this.uploadForm.get('profile').value)
        // console.log('formdata', formData);

        for (let i = 0; i < this.uploadForm.get('profile').value.length; i++) {
          this.filenames.push(this.uploadForm.get('profile').value[i].name);
          console.log(`Uploading ${this.uploadForm.get('profile').value[i].name} with size ${this.uploadForm.get('profile').value[i].size} and type ${this.uploadForm.get('profile').value[i].type}`);
          // this.uploadService.uploadFile(files[i], files[i].name);
          this.screensaverService.upload(this.uploadForm.get('profile').value[i], id).subscribe(
            res => {
              console.log('Finished writing', res);
