...
export class UpdateJoueurAcademieValidationComponent implements OnInit {
  ...
  upload(event) {
    this.showViaServiceWait("salaire");
    this.clicked = false;
    for (let file of event.files) {
      this.uploadedFiles.push(file);
      let formData = new FormData();
      formData.append("file", file);
      this.fileService.uploadMontantAvanceSalaire(formData).subscribe(
        resp => {
          console.log(resp);
          this.showViaServiceOk(resp, "salaire");
          this.clicked = true;
        },
        err => {
          console.log(err)
          this.showViaServiceEror("salaire");
          this.clicked = true;
        }
      );
    }
  ...
  showViaServiceOk() {
    this.messageService.clear();
    this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Message Content : nombre de lignes insérées ' });
  }

  showViaServiceEror(key: string) {
    this.messageService.clear();
    this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Message Content : Problème d\'insérision' });
  }
