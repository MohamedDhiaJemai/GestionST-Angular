...
export class UpdateJoueurAcademieValidationComponent implements OnInit {
  ...
  viewFile(id: number, name: string) {
    this.docuementsJoueurService.viewFile(id, name).subscribe(
      res => {
  ...
  showViaServiceOk(resp: string) {
    this.messageService.clear();
    this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Message' + resp });
  }

  showViaServiceEror(resp: string) {
    this.messageService.clear();
    this.messageService.add({ severity: 'error', summary: 'Error', detail: 'Message' + resp });
  }
