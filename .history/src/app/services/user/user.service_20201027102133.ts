...
export class UserService {
  ...
  findUserById(id) {
    if (this.jwtToken == null) { this.loadToken(); }
    if (this.jwtHelper.isTokenExpired(this.jwtToken)) { this.addToken(); }
    return this.httpClient.get<Utilisateur>(this.apiUrl + '/idUtilisateur/' + id,
  ...
  getAllInformationAboutUser() {
    ...
    if (this.jwtHelper.isTokenExpired(this.jwtToken)) { this.addToken(); }
    // console.log('UTILISATEUR SERVICE : ', this.jwtToken);
    return this.httpClient.get<Utilisateur>(this.apiUrl + '/informationUser',
      { headers: new HttpHeaders({ 'authorization': this.jwtToken }) });
  }
  ...
  ListOfUsersByCentre() {
    ...
    if (this.jwtHelper.isTokenExpired(this.jwtToken)) { this.addToken(); }
    // console.log('UTILISATEUR SERVICE : ', this.jwtToken);
    return this.httpClient.get<Utilisateur[]>(this.apiUrl + '/ListOfUsersByCentre', 
    { headers: new HttpHeaders({ 'authorization': this.jwtToken }) });
  }
}
