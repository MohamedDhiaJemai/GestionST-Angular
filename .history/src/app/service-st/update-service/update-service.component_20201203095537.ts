        ...
        if (data === null) {
          this.serviceSTService.findByIdComplement(id).subscribe(
            (datac: ServiceSTC) => {
              ...
              this.selectedJour = Object.keys(this.serviceSTC.jours).map(key => ({ label: jour[key], value: key }));
              console.log('jourrrr', this.selectedJour)
              // this.serviceSTC.jours.forEach(element => {
              //   this.jours.map((element) => this.selectedJour.push(element));
              // });

              // this.serviceSTC.jours.map((item) => this.selectedJour.push(key => ({ label: item[key], value: key })));
