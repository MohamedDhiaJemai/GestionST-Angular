        ...
        if (data === null) {
          this.serviceSTService.findByIdComplement(id).subscribe(
            (datac: ServiceSTC) => {
              ...
              this.selectedJour = Object.keys(this.serviceSTC.jours).map(key => ({ label: this.serviceSTC.jours[key], value: key }));
              console.log('jourrrr', this.selectedJour)

              this.serviceSTC.jours.forEach(element => {
                this.selectedCars = [this.cars[element].value]
                this.selectedJour.push(element);
              });

              // this.selectedCars  = [this.cars[2].value]
            }
          );
        } else {
          this.test = false;
